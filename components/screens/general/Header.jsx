...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIsSearching(true);
      axiosPublic
        .get(`public/products/?q=${encodeURIComponent(searchValue.trim())}`)
        .then((res) => {
  ...
  const searchDropdownUI = showSearchDropdown && (
    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl z-50 overflow-hidden flex flex-col">
      {isSearching ? (
        <div className="p-4 text-center text-sm text-gray-500 animate-pulse font-bold">
          Searching…
        </div>
        ...
        <div className="flex flex-col max-h-[350px] overflow-y-auto">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <p className="text-xs text-teal-600 font-black">
                    ₹{product.lowest_variant_price ?? product.price}
                  </p>
                  {(product.lowest_variant_mrp ?? product.mrp) >
                    (product.lowest_variant_price ?? product.price) && (
          ...
          <div className="bg-gray-50 border-t border-gray-100 px-4 py-2 text-center sticky bottom-0">
            <button
              ...
              className="text-xs font-bold text-teal-600 hover:text-teal-700 w-full py-1 focus:outline-none"
            >
      ...
      <header className="header-root w-full bg-white shadow-sm sticky top-0 z-50">
        ...
        <div className="px-4 sm:px-6 py-3 flex items-center gap-3">
          ...
          <Link href="/" className="flex-shrink-0">
            ...
            />
            <span
              className="text-teal-600 font-black text-lg hidden"
              style={{ display: "none" }}
            >
              BabyStore
            </span>
          ...
          <div className="flex items-center gap-4 ml-auto">
            {user && (
              <Link
                href="/wishlist"
                className="hidden sm:flex flex-col items-center gap-0.5 text-gray-500 hover:text-teal-500 transition-colors"
              >
                <HeartIcon />
            ...
            {user && (
              <Link
                href="/cart"
                className="flex flex-col items-center gap-0.5 text-gray-500 hover:text-teal-500 transition-colors relative"
              >
                <CartIcon />
                {cartCount > 0 && (
          ...
          <nav className="flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className={`nav-ul flex items-center gap-1.5 px-3 py-2 text-sm font-bold rounded-md transition-colors ${
                  link.hot
          ...
          <div className="sm:hidden border-t border-gray-100 bg-white">
            <div className="px-4 pt-4 pb-2 relative" ref={searchDropdownRef}>
              <div className="flex items-center border-2 border-teal-400 rounded-lg overflow-hidden bg-white">
                ...
                <button
                  onClick={handleSearch}
                  className="px-4 py-2.5 text-gray-400 border-l border-gray-200"
                >
                  <SearchIcon />
                </button>
              </div>
